/**
 * REAL PROVIDER: Google Gemini (free tier) via REST — no SDK, to keep things lightweight.
 * https://aistudio.google.com/apikey
 *
 * Product rule enforced at the code level, not just in the prompt: the returned
 * `disclaimer` is ALWAYS the local constant, never text generated by the model — so
 * even if the model "forgot" the instruction, the app never shows a price as final.
 */
...
Max 5 suggestions, max 4 missingInfo items.`;
    ...
    return {
      ...
      missingInfo: Array.isArray(json.missingInfo) ? json.missingInfo.slice(0, 4) : [],
      priceRangeHint,
      disclaimer: DISCLAIMER // never the text generated by the model
    };
  }
